...
| **Usage / Debug** | Inspect token-usage coverage and trends; Usage opens on the current local calendar day (`Today`) by default, with range controls ordered `Today`, `7d`, `30d`, then `Available history`. The Usage report refreshes every five seconds while open. `Today` covers the current local calendar day and shows hourly activity bars; `7d` shows daily activity bars. Both bar charts include cache-hit percentages when normalized cache-read and input-token data is available, and `—` when it is unavailable. The Models table shows the same per-model cache-hit percentages. Its `tok/s` column shows aggregate output tokens per second, calculated as output tokens divided by the full request duration (not with TTFT subtracted and not as an arithmetic mean of per-request rates); `~` marks an aggregate containing estimated usage, and `—` means no valid speed sample is available. It also shows upstream provider weekly quota utilization and reset information when reported, or enables opt-in provider transport and usage-extraction diagnostics. Unavailable, malformed, or failed quota data appears as `—` and does not block the local usage report; this is upstream provider quota, not reconstructed local seven-day activity. |
