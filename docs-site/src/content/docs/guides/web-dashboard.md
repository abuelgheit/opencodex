...
| **Usage / Debug** | Inspect token-usage coverage and trends, see upstream provider weekly quota utilization and reset information when reported, or enable opt-in provider transport and usage-extraction diagnostics. Unavailable, malformed, or failed quota data appears as `—` and does not block the local usage report; this is upstream provider quota, not reconstructed local seven-day activity. |
